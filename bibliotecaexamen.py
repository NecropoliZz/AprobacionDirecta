...
def obtener_posiciones_de_estudiantes(estudiante:str,estudiantes:list)->list:
    """
...
def mostrar_calificaciones_de_estudiantes(estudiantes:list,posiciones:list,calificaciones:list):
    ...
    if len(posiciones) != 0:
        for indice in posiciones:
            print(f"Notas de {estudiantes[indice]}:",end=" ")
            for nota in calificaciones[indice]:
                print(nota,end=" ")
            print()
    else:
       print("No se encontro ningun estudiante con ese nombre")
def obtener_posiciones_de_nota(nota:int,calificaciones:list)->list:
    """
...
def mostrar_estudiantes_de_una_nota(nota:int,posiciones:list,estudiantes:list,materias:list):
    ...
    if len(posiciones) != 0:
      for lista in posiciones:
          fila = lista[0]
          columna = lista[1]
          print(f"Nota encontrada: {nota} -> Estudiante: {estudiantes[fila]}, Materia: {materias[columna]}")
    else:
